...
def getModel():
    ...
    model.add(Dense(50, input_dim=16, use_bias=True, activation='sigmoid', init='random_uniform'))
    #model.add(Dropout(0.20))
    model.add(Dense(50, activation='sigmoid', use_bias=True, init='random_uniform'))
    #model.add(Dropout(0.20))    
    model.add(Dense(5, init='random_uniform', use_bias=True, activation='softmax'))
...
class BrainImpl(brain_capnp.Brain.Server):
    def __init__(self):
        ...
        for x in range(0,433): #433
            temp_expected,temp_example,temp_counts = pickle.load(open("../packets/save-{0}.p".format(x), "rb"))
            sys.stdout.write('.')
        ...
        for y in range(1,401):
                ...
                slice_start = y*len(self.expected)/1000
                slice_size = len(self.expected)/1000 + slice_start
                a = [x for x in islice(self.example, slice_start, slice_size)]
                b = [x for x in islice(self.expected, slice_start, slice_size)]
                c = [x for x in islice(self.count, slice_start, slice_size)]
                data = Dataset(a, b, c)
		print len(data.training_example)
                print len(data.training_expected)
                self.model.fit(np.array(data.training_example), np.array(data.training_expected), batch_size=1, nb_epoch=5)
                #self.store_results(c, a)
                self.store_results(data.test_counts, data.test_example)
                ...
                with open("output-window-perf.txt", "a") as myfile:
    ...
    def store_results(self, counts, examples):
        ...
        with open("output-window.txt", "a") as myfile:
