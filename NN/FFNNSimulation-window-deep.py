...
def getModel():
    ...
    model.add(Dense(50, input_dim=16, activation='sigmoid', init='uniform'))
...
class BrainImpl(brain_capnp.Brain.Server):
    def __init__(self):
		...
		print len(data.training_example)
                ...
                self.model.fit(np.array(data.training_example), np.array(data.training_expected), batch_size=1, nb_epoch=1)
                ...
                with open("output-window-deep-perf.txt", "a") as myfile:
    ...
    def store_results(self, counts, examples):
        ...
        with open("output-window-deep.txt", "a") as myfile:
